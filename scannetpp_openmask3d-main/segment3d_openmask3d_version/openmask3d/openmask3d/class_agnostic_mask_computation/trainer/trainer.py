...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        elif self.config.data.validation_dataset.dataset_name=='scannetpp':
            from scannetpp_constants import SCANNETPP_CLASS_LABLES
            ids = np.arange(1, 2754)
            lbls = SCANNETPP_CLASS_LABLES
            self._labels = {lbl_id:{'name':lbl} for (lbl_id, lbl) in zip(ids, lbls)}
    ...
    def get_class_agn_mask(self, mask_cls, mask_pred, num_queries=100, num_classes=2753, device=None):
    ...
    def eval_instance_step(self, output, target_low_res, target_full_res, inverse_maps, file_names,
        ...
        if self.config.data.validation_dataset.dataset_name == "scannet200" or self.config.data.validation_dataset.dataset_name == "scannetpp":
        ...
        for bid in range(len(prediction[self.decoder_id]['pred_masks'])):
            ...
            np.save(os.path.join(pred_save_folder, file_names[bid]+"_masks.npy"), self.preds[file_names[bid]]['pred_masks'].astype(np.float16))
