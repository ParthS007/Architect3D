...
import tqdm
import argparse
from scannetpp_constants import SCANNETPP_CLASS_LABLES
...
class InstSegEvaluator():
    ...
    def get_query_sentences(self, dataset_type, sentence_structure="a {} in a scene"):
        if dataset_type == 'scannet':
            ...
            label_list = list(SCANNETPP_CLASS_LABLES)
        else:
    ...
    def set_label_and_color_mapper(self, dataset_type):
        if dataset_type == 'scannet':
            ...
            file_path = "/work/scratch/dbagci/processed/scannetpp/label_database.yaml"
            import yaml
            with open(file_path, 'r') as f:
                data = yaml.safe_load(f)

            # Build color_map: {class_id: color}
            color_map = {class_id: attributes['color'] for class_id, attributes in data.items()}
    ...
    def compute_classes_per_mask_diff_scores(self, masks_path, mask_features_path, keep_first=None):
        ...
        print(self.text_query_embeddings.shape)
...
def test_pipeline_full_scannet200(mask_features_dir,
                                    ...
                                    keep_first = None,
                                    scene_list_file='evaluation/nvs_sem_val.txt',
    ...
    for scene_name in tqdm.tqdm(scene_names[:]):
        ...
        # /work/courses/3dv/20/OpenArchitect3D/mask_features_scannetpp
        scene_per_mask_feature_path = os.path.join(mask_features_dir, "/work/courses/3dv/20/OpenArchitect3D/mask_features_scannetpp/scene"+feature_file_template.format(scene_name))
