...
def get_number_of_images(poses_path):
    i = 0
    while(os.path.isfile(os.path.join(poses_path, f"{i:06d}.txt"))): i += 1
    return i
...
class Camera:
    ...
    def load_poses(self, indices):
        path = os.path.join(self.poses_path, "000000" + '.txt')
        shape = np.linalg.inv(np.loadtxt(path))[:3, :].shape
        poses = np.zeros((len(indices), shape[0], shape[1]))
        for i, idx in enumerate(indices):
            path = os.path.join(self.poses_path, f"{idx:06d}" + '.txt')
...
class Images:
    ...
    def load_images(self, indices):
        ...
        for idx in indices:
            #f"{idx:06d}.txt"
            img_path = os.path.join(self.images_path, f"{idx:06d}" + self.extension)
