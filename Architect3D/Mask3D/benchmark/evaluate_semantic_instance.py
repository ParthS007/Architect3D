...
#<GROUP20 START>
config_path = "/work/courses/3dv/20/OpenArchitect3D/Mask3D/conf/config_base_instance_segmentation.yaml"
with open(config_path, "r") as f:
    config = yaml.safe_load(f)

if "general" in config:
    opt.update(config["general"])
#<GROUP20 END>
...
def make_pred_info(pred: dict):
    ...
    for i in range(len(pred["pred_classes"])):
        info = {}
        info["label_id"] = pred["pred_classes"][i] #0
...
def assign_instances_for_scan(pred: dict, gt_file: str):
    ...
    #print("#################### PRED INFO",pred_info)
    #print("LEN CLASS LABELS", len(CLASS_LABELS))

    if opt.get("eval_class_agnostic", False):
        gt_instances = {"object": []}
        
        # Get all instance IDs from the ground truth
        ...
        for instance_id in unique_ids:
            ...
            gt_instance = {}
            gt_instance["instance_id"] = instance_id
            gt_instance["label_id"] = 0  # All instances belong to class "object"
            gt_instance["vert_count"] = np.sum(gt_ids == instance_id)
            gt_instance["med_dist"] = 0  # Not used for class-agnostic evaluation
            gt_instance["dist_conf"] = 2  # Not used for class-agnostic evaluation
            gt_instances["object"].append(gt_instance)
    #<GROUP20 END>
...
def evaluate(
    ...
    if dataset == "scannetpp" and opt.get("eval_class_agnostic", False):
