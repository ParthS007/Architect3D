...
class InstanceSegmentation(pl.LightningModule):
    ...
    #<GROUP20 START> (Honestly not sure anymore what exactly was changed, so I marked the whole function)
    ...
    def eval_instance_step(
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            # save predictions
            #if file_names[bid] == "0d2ee665be":
            #pred_save_folder = "/work/courses/3dv/20/OpenArchitect3D/Mask3D/masks_baseline"
            ...
            if self.config.general.save_visualizations and file_names[bid] == "0d2ee665be":
